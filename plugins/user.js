...
Bixby(
  ...
  async (message) => {
    ...
    try {
      await savePausedChat(chatId); // Use the exported function
...
Bixby(
  ...
  async (message) => {
    ...
    try {
      const pausedChat = await PausedChats.findOne({ chatId }); // Use Mongoose's findOne

      if (pausedChat) {
        await pausedChat.deleteOne(); // Use Mongoose's deleteOne
...
Bixby(
  ...
  async (message) => {
    try {
      await deleteAllPausedChats(); // Use the exported function
...
Bixby(
  ...
  async (message, match) => {
    ...
    // Extract reason from the message or reply
    let reason = message?.reply_message?.text || match;
    reason = reason?.replace(/@(\d+)/, ""); // Remove mentions from the reason
    reason = reason?.trim() || "Reason not Provided"; // Default reason if not provided

    // Save the warn to the database
    const warnInfo = await saveWarn(userId, reason);
    let userWarnCount = warnInfo ? warnInfo.warnCount : 0;

    // Send a reply with the warn details
    await message.reply(
      `_User @${
        userId.split("@")[0]
    ...
    // Check if the user has exceeded the warn limit
...
// Bixby command to reset warnings for a user
Bixby(
  {
    pattern: "resetwarn",
    fromMe: isPrivate,
  ...
  async (message) => {
    ...
    // Reset the warnings for the user
